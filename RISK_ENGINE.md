...
```
score = clamp( 50 + Σ signal.weight , 0 , 100 )        # rounded to an integer
```
...
`test_every_scoring_signal_has_evidence` enforces this for all four demo scenarios.

## Worked examples (the four demo scenarios)
...
- **Heuristics, not a verdict.** Thresholds (30/180/365 days, 100/1000 transactions, 20%/50% failures) are reasonable defaults, not calibrated against labelled data.
...
- **Ethereum only.** Ethereum Mainnet and Sepolia. No other chains.
