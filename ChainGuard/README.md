...
- **Deterministic scoring.** `score = clamp(50 + Σ signal weights, 0, 100)`. Fixed rules, fixed weights (`risk_engine/signals.py`). Same input → same score. **No LLM or ML computes or influences the score.**
...
- Chains: **Ethereum Mainnet and Sepolia only** (no other chains)
...
deterministic synthetic data. The UI shows a demo banner, four one-click
...
```
USE_DEMO_MODE=false
ETH_MAINNET_ETHERSCAN=<your Etherscan API key>
# ETH_SEPOLIA_ETHERSCAN=<key>  (a single Etherscan V2 key works for all chains)
```
...
- The browser page always analyzes Ethereum Mainnet; Sepolia is available through the API (`"chain": "sepolia"`).
...
`chain` is `ethereum_mainnet` (default) or `sepolia`. Response shape (abridged):
...
`score = clamp(50 + Σ weights, 0, 100)` — risk levels: 0–20 critical · 21–40 high · 41–60 moderate · 61–80 low · 81–100 very low.
...
80+ tests, run against the real FastAPI app (via `TestClient`), covering:
address validation; every signal rule; deterministic scoring and risk-level
boundaries; the four demo scenarios end to end (exact scores, evidence on every
...
- **Ethereum Mainnet and Sepolia only.** No multi-chain analysis.
