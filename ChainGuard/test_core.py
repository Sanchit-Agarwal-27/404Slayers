...
EXPECTED = {
    ...
    "review2_anomaly_wallet": (
        11,
        "critical",
        {"Transaction Value Outlier", "Gas Price Anomaly", "Counterparty Concentration",
         "Unusual Token Approval", "Rapid Pass-Through Pattern", "Circular Transaction Pattern"},
    ),
}


class TestDemoScenarios:
    def test_demo_addresses(self):
...
class TestDocs:
    ...
    def test_readme_makes_no_unimplemented_claims(self):
        ...
        assert "multi-chain aggregation" in readme
        assert "demo mode intentionally remains single-chain" in readme
...
# ============================================================================
# Review 2 enhancements: statistical / behavioral signals and multi-chain
# ============================================================================


class TestReview2Signals:
    def test_transaction_value_outlier(self):
        txs = [make_tx(value=10.0, n=0)] + [make_tx(value=1.0, n=i) for i in range(1, 6)]
        signals, _ = SignalExtractor().extract_signals(WALLET, txs, is_contract=False)
        sig = next(s for s in signals if s.name == "Transaction Value Outlier")
        assert sig.weight == SIGNAL_WEIGHTS["transaction_value_outlier"]
        assert sig.evidence["iqr_upper_bound"] < sig.evidence["current_value"]
        assert sig.evidence["historical_sample_count"] == 5

    def test_gas_price_anomaly(self):
        txs = [make_tx(value=1, n=0) for _ in range(6)]
        for i, tx in enumerate(txs):
            tx.gas_price = 500.0 if i == 0 else 10.0
            tx.hash = "0x" + format(100 + i, "064x")
        signals, _ = SignalExtractor().extract_signals(WALLET, txs, is_contract=False)
        sig = next(s for s in signals if s.name == "Gas Price Anomaly")
        assert sig.weight == SIGNAL_WEIGHTS["gas_price_anomaly"]
        assert sig.evidence["iqr_upper_bound"] < sig.evidence["current_gas_price_gwei"]

    def test_no_statistical_signal_with_insufficient_history(self):
        txs = [make_tx(value=100.0, n=0), make_tx(value=1.0, n=1)]
        signals, _ = SignalExtractor().extract_signals(WALLET, txs, is_contract=False)
        assert not any(s.name in {"Transaction Value Outlier", "Gas Price Anomaly"} for s in signals)

    def test_zero_variance_does_not_crash_or_flag(self):
        txs = [make_tx(value=1.0, n=i) for i in range(6)]
        signals, _ = SignalExtractor().extract_signals(WALLET, txs, is_contract=False)
        assert not any(s.name in {"Transaction Value Outlier", "Gas Price Anomaly"} for s in signals)

    def test_counterparty_concentration(self):
        ...
        txs = [make_tx(frm=WALLET, to=a, value=90, n=1), make_tx(frm=WALLET, to=b, value=5, n=2), make_tx(frm=WALLET, to=c, value=5, n=3)]
        signals, _ = SignalExtractor().extract_signals(WALLET, txs, is_contract=False)
        sig = next(s for s in signals if s.name == "Counterparty Concentration")
        assert sig.weight == SIGNAL_WEIGHTS["counterparty_concentration"]
        assert sig.evidence["top_counterparty"] == a
        assert sig.evidence["top_counterparty_share"] == 0.9

    def test_unlimited_approval_decoding(self):
        spender = "0xdddddddddddddddddddddddddddddddddddddddd"
        selector = "095ea7b3"
        spender_word = spender[2:].rjust(64, "0")
        amount_word = format(2**256 - 1, "064x")
        tx = make_tx(frm=WALLET, to="0xeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeee", value=0, n=9)
        tx.input_data = "0x" + selector + spender_word + amount_word
        signals, _ = SignalExtractor().extract_signals(WALLET, [tx], is_contract=False)
        sig = next(s for s in signals if s.name == "Unusual Token Approval")
        assert sig.evidence["function"] == "approve"
        assert sig.evidence["spender"] == spender
        assert sig.evidence["unlimited_style"] is True

    def test_rapid_pass_through(self):
        incoming = make_tx(frm=OTHER, to=WALLET, value=100, days_ago=1, n=1)
        outgoing = make_tx(frm=WALLET, to=OTHER, value=95, days_ago=1, n=2)
        outgoing.timestamp = incoming.timestamp + timedelta(minutes=3)
        signals, _ = SignalExtractor().extract_signals(WALLET, [outgoing, incoming], is_contract=False)
        sig = next(s for s in signals if s.name == "Rapid Pass-Through Pattern")
        assert sig.evidence["time_difference_seconds"] == 180

    def test_direct_round_trip_detection(self):
        counterparty = OTHER
        outgoing = make_tx(frm=WALLET, to=counterparty, value=10, days_ago=1, n=1)
        incoming = make_tx(frm=counterparty, to=WALLET, value=9, days_ago=1, n=2)
        incoming.timestamp = outgoing.timestamp + timedelta(hours=2)
        signals, _ = SignalExtractor().extract_signals(WALLET, [outgoing, incoming], is_contract=False)
        sig = next(s for s in signals if s.name == "Circular Transaction Pattern")
        assert sig.evidence["cycle"] == [WALLET, counterparty, WALLET]


class TestMultiChain:
    def test_supported_evm_networks_are_configured(self):
        expected = {"ethereum_mainnet", "sepolia", "polygon", "arbitrum", "optimism", "base", "bsc"}
        assert expected.issubset(NETWORKS.keys())
        assert {NETWORKS[k]["chain_id"] for k in expected} == {1, 11155111, 137, 42161, 10, 8453, 56}

    def test_multichain_request_validates_evm_address(self):
        from core.models import MultiChainAnalysisRequest, Chain
        request = MultiChainAnalysisRequest(address=LOW_RISK_ADDRESS, chains=[Chain.ETHEREUM_MAINNET, Chain.POLYGON])
        assert request.address == LOW_RISK_ADDRESS
        assert [c.value for c in request.chains] == ["ethereum_mainnet", "polygon"]

    def test_multichain_endpoint_requires_live_mode(self, client):
        r = client.post("/api/analyze/multichain", json={"address": LOW_RISK_ADDRESS, "chains": ["ethereum_mainnet", "polygon"]})
        assert r.status_code == 400
        assert "USE_DEMO_MODE=false" in r.json()["detail"]
