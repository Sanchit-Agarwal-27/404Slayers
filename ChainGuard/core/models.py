...
class MultiChainAnalysisRequest(BaseModel):
    """Analyze one EVM address across one or more supported chains."""

    address: str = Field(..., description="EVM address")
    chains: List[Chain] = Field(default_factory=lambda: [Chain.ETHEREUM_MAINNET, Chain.POLYGON, Chain.ARBITRUM])
    ...
    def validate_address(cls, v: str) -> str:
        v = v.strip()
        if not v.startswith("0x") or len(v) != 42 or not all(c in "0123456789abcdefABCDEF" for c in v[2:]):
            raise ValueError("Invalid EVM address format. Must be 0x followed by 40 hex characters.")
        return v.lower()


class MultiChainAnalysisResponse(BaseModel):
    """Per-chain results plus a compact cross-chain summary."""

    address: str
    results: Dict[str, "AddressAnalysisResponse"]
