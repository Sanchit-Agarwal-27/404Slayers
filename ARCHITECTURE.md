...
├── blockchain/
│   ├── provider.py              # BlockchainProvider ABC + Transaction / Contract dataclasses
│   ├── demo_provider.py         # Deterministic offline provider + the 4 demo scenarios
│   └── eth_provider.py          # Live provider (Etherscan API V2, aiohttp)
├── risk_engine/
...
│   ├── analyze.py               # POST /api/analyze, GET /api/analyze/demo/{scenario}
...
1. **Validate** — `AddressAnalysisRequest` (Pydantic) checks `0x` + 40 hex characters, lower-cases the address, and constrains `chain` to `ethereum_mainnet` or `sepolia`. Failures return **422**.
...
| `POST /api/analyze` | Analyze an address (body: `address`, `chain`, `include_transactions`, `include_graph`) | 200; 400 live mode without key; 422 invalid input; 502 upstream failure |
...
- `DemoProvider` holds four synthetic scenarios (addresses defined once in `demo_provider.py` and imported elsewhere). Unknown addresses return empty data.
- `EthereumProvider` uses the **Etherscan API V2** (`https://api.etherscan.io/v2/api` with a `chainid` parameter). The V1 endpoints were shut down on 2025-08-15. Contract detection uses `eth_getCode` (not `getsourcecode`, which would misclassify *unverified* contracts as wallets). Timestamps are converted to naive UTC to match the risk engine. Only Ethereum Mainnet (chain 1) and Sepolia (11155111) are configured.
...
Settings come from environment variables / `.env` (see `.env.example`); unknown keys are ignored. Relevant to behavior: `USE_DEMO_MODE`, `ETH_MAINNET_ETHERSCAN`, `ETH_SEPOLIA_ETHERSCAN`, `PROVIDER_TIMEOUT_SECONDS`, `MAX_COUNTERPARTY_CLASSIFICATION_CALLS`, `DB_PATH`, `HOST`, `PORT`, `DEBUG`.
